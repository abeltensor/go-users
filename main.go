...
func login(w http.ResponseWriter, r *http.Request) {
	...
	if name != "" && pass != "" {
		if userExists(u) {
			...
			setFlash(w, "message", []byte("Input a valid username and password!"))
		}
	} else {
		setFlash(w, "message", []byte("Please input a username and password!"))
	}
...
func examplePage(w http.ResponseWriter, r *http.Request) {
	...
	if username != "" {
		...
		setFlash(w, "message", []byte("Please Login!"))
		http.Redirect(w, r, "/", 302)
	}
}
